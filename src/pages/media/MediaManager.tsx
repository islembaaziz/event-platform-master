...
const MediaManager = () => {
  ...
  useEffect(() => {
    const fetchMedia = async () => {
      try {
        ...
        toast.error(error.response?.data?.message || 'Failed to load media items');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const updateTag = async (itemId: string, oldTag: string, newTagValue: string) => {
    ...
    try {
      ...
      const updatedTags = item.tags.map(tag => tag === oldTag ? newTagValue : tag);
      const { data } = await api.put(`/media/${itemId}`, { tags: updatedTags });
  ...
  const deleteSelectedItems = async () => {
    ...
    try {
      ...
      setMediaItems(prev => prev.filter(item => !selectedItems.includes(item._id)));
      setSelectedItems([]);
      ...
      <div className="space-y-6">
        ...
        <div 
          ...
          className={`border-2 border-dashed rounded-lg p-8 text-center cursor-pointer transition-colors ${
            isDragActive ? 'border-primary-500 bg-dark-200' : 'border-dark-300 hover:border-primary-500'
          }`}
        >
          <input {...getInputProps()} />
        ...
        <div className="bg-dark-100 p-4 rounded-lg">
          <div className="flex flex-wrap gap-4 items-center justify-between">
            ...
            <div className="flex flex-wrap gap-3 items-center">
              <div className="relative">
                ...
                <div className="absolute right-0 top-full mt-1 bg-dark-100 rounded-md shadow-lg z-10 w-60 py-2 border border-dark-300 hidden group-hover:block">
                  <div className="px-3 py-2">
                    <p className="text-xs text-dark-500 mb-2">Select tags to filter</p>
                    <div className="space-y-1 max-h-48 overflow-y-auto">
                      {allTags.map(tag => (
                        <label key={tag} className="flex items-center p-1 hover:bg-dark-200 rounded cursor-pointer">
                          <input
                            type="checkbox"
              ...
              <div className="bg-dark-200 rounded-md flex overflow-hidden">
                <button
                  className={`p-2 ${viewMode === 'grid' ? 'bg-primary-500 text-dark' : 'text-dark-500'}`}
                ...
                <button
                  className={`p-2 ${viewMode === 'list' ? 'bg-primary-500 text-dark' : 'text-dark-500'}`}
          ...
          {selectedTags.length > 0 && (
            <div className="mt-3 flex flex-wrap gap-2">
              {selectedTags.map(tag => (
                <div key={tag} className="bg-dark-200 text-white text-xs py-1 px-2 rounded-full flex items-center">
                  <span>{tag}</span>
                  <button 
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {item.tags.map(tag => (
                        <div key={tag} className="group relative">
                          {editingTag && editingTag.id === item._id && editingTag.tag === tag ? (
                            <div className="flex items-center bg-dark-200 rounded-full overflow-hidden">
                              <input
                                type="text"
                                value={editingTag.tag}
                                onChange={(e) => setEditingTag({ ...editingTag, tag: e.target.value })}
                                className="bg-transparent border-none text-white text-xs py-0.5 px-2 w-20 focus:outline-none"
                                autoFocus
                              />
                              <div className="flex">
                                <button 
                                  onClick={() => updateTag(item._id, tag, editingTag.tag)}
                            ...
                            <div className="bg-dark-200 text-white text-xs py-0.5 px-2 rounded-full flex items-center">
                              <Tag className="h-3 w-3 mr-1 text-primary-500" />
                              <span>{tag}</span>
                              <div className="hidden group-hover:flex ml-1">
                                <button 
                                  className="hover:text-primary-500 p-0.5"
                      ...
                      {item.tags.length < 5 && (
                        <button 
                          className="bg-dark-200 text-dark-500 hover:text-white text-xs py-0.5 px-2 rounded-full flex items-center"
                          onClick={() => {
                            setEditingTag({ id: item._id, tag: '' });
                          }}
                        >
          ...
          <div className="bg-dark-100 rounded-lg overflow-hidden">
            <table className="min-w-full divide-y divide-dark-300">
              ...
              <tbody className="divide-y divide-dark-300">
                {filteredItems.map(item => (
                  <tr key={item._id} className="hover:bg-dark-200">
                    <td className="p-3">
                      <input
                        type="checkbox"
                        checked={selectedItems.includes(item._id)}
                    ...
                    <td className="p-1">
                      <div className="h-12 w-12 bg-dark-200 rounded overflow-hidden">
                        {item.type === 'image' ? (
                          <img 
                            src={item.url} 
                            alt={item.name} 
                          ...
                          <div className="flex items-center justify-center h-full">
                            <Video className="h-6 w-6 text-dark-500" />
                          </div>
                        )}
                      </div>
                    </td>
                    <td className="p-3 text-sm text-white">
                      {item.name}
                      {item.eventId && (
                        <p className="text-xs text-dark-500">{item.eventId}</p>
                      )}
                    </td>
