...
export const getAllEvents = async (req, res) => {
  try {
    ...
    if (userId) {
      events = events.filter(event => event.createdBy === userId);
    }
    ...
    const eventsWithCounts = await Promise.all(
      events.map(async (event) => {
        const [likesCount, commentsCount] = await Promise.all([
          ...
          Comment.countDocuments({ targetType: 'Event', targetId: event._id, approved: true })
        ]);
...
export const updateEvent = async (req, res) => {
  try {
    ...
    if (event.createdBy.toString() !== req.user._id.toString() && !['administrator'].includes(req.user.role)) {
...
export const likeEvent = async (req, res) => {
  try {
    ...
    if (existingLike) {
      ...
      const likesCount = await Like.countDocuments({ targetType: 'Event', targetId: event._id });
      res.json({ message: 'Event unliked successfully', liked: false, likesCount });
    } else {
      // Like - add the like
      await Like.create({
        user: req.user._id,
      ...
      const likesCount = await Like.countDocuments({ targetType: 'Event', targetId: event._id });
      res.json({ message: 'Event liked successfully', liked: true, likesCount });
    }
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: 'Server error' });
