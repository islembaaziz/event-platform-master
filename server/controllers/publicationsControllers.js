...
export const getAllPublications = async (req, res) => {
  try {
    ...
    // Get like and comment counts for each publication
    const publicationsWithCounts = await Promise.all(
      publications.map(async (publication) => {
        const [likesCount, commentsCount] = await Promise.all([
          Like.countDocuments({ targetType: 'Publication', targetId: publication._id }),
          Comment.countDocuments({ targetType: 'Publication', targetId: publication._id, approved: true })
        ]);
...
export const getPublicationById = async (req, res) => {
  try {
    ...
    const [likesCount, commentsCount, comments] = await Promise.all([
      Like.countDocuments({ targetType: 'Publication', targetId: publication._id }),
      Comment.countDocuments({ targetType: 'Publication', targetId: publication._id, approved: true }),
      Comment.find({ targetType: 'Publication', targetId: publication._id, approved: true })
        .populate('user', 'name')
        .sort({ createdAt: -1 })
        .limit(10)
    ]);
...
export const updatePublication = async (req, res) => {
  try {
    ...
    if (publication.author.toString() !== req.user._id.toString() && !['administrator'].includes(req.user.role)) {
      return res.status(403).json({ message: 'Not authorized to update this publication' });
    }
    
    const isNewlyPublished = publication.status !== 'published' && req.body.status === 'published';
...
export const deletePublication = async (req, res) => {
  try {
    ...
    await Promise.all([
      ...
      Comment.deleteMany({ targetType: 'Publication', targetId: publication._id })
    ]);
...
export const likePublication = async (req, res) => {
  try {
    ...
    if (existingLike) {
      ...
      const likesCount = await Like.countDocuments({ targetType: 'Publication', targetId: publication._id });
      res.json({ message: 'Publication unliked successfully', liked: false, likesCount });
    } else {
      // Like - add the like
      await Like.create({
        user: req.user._id,
      ...
      const likesCount = await Like.countDocuments({ targetType: 'Publication', targetId: publication._id });
      res.json({ message: 'Publication liked successfully', liked: true, likesCount });
    }
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: 'Server error' });
